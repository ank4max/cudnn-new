...
template<class T>
Her2k<T>::Her2k(size_t A_row, size_t A_col, size_t B_row, size_t B_col, size_t C_row, size_t C_col, T alpha, double beta, char mode)
...
int Her2k<T>::Her2kApiCall() {
  ...
  /**
   * API call to performs Hermitian rank-2k update : \f$ C = alpha * A * B^H + alpha * B * A^H  + beta * C \f$
   */
  ...
  switch (mode) {
    case 'C': {
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  XtCher2k kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "XtCher2k API call ended\n";
      break;
    }
  
    case 'Z': {
      std::cout << "\nCalling XtZher2k API\n";
      clk_start = clock();

      status = cublasXtZher2k(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  XtZher2k kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "XtZher2k API call ended\n";
      break;
    }
  }
  
  std::cout << "\nMatriz C after Xt" << mode << "her2k operation is:\n";
