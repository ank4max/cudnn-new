...
int zgemm(int A_row, int A_col, int B_row, int B_col, int C_row, int C_col, float alpha_real, float beta_real) {
  clk_start = 0;
  clk_end = 0;
  float alpha_imaginary = 0.0f;
  float beta_imaginary = 0.0f; 
  ...
  cuDoubleComplex*HostMatA; // mxk matrix A on the host
  cuDoubleComplex*HostMatB; // kxn matrix B on the host
  cuDoubleComplex*HostMatC; // mxn matrix C on the host
  
  HostMatA = new cuDoubleComplex[A_row * A_col]; // host memory for A
  HostMatB = new cuDoubleComplex[B_row * B_col]; // host memory for B
  HostMatC = new cuDoubleComplex[C_row * C_col]; // host memory for C
  ...
  // on the device
  cuDoubleComplex*DeviceMatA; // d_A - A on the device
  cuDoubleComplex*DeviceMatB; // d_B - B on the device
  cuDoubleComplex*DeviceMatC; // d_C - C on the device
  ...
  std::cout << "\nMatriz C after zGemm operation is:\n";
  PrintCuMatrix(HostMatC, C_row, C_col); 
