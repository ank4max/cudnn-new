...
int Convolution::ConvolutionBackwardApiCall() {
  ...
  int output_size = (height + filter_height-1) * (height + filter_height-1);
  ...
  status = cudnnSetTensor4dDescriptor(output_desc, data_format, data_type, batch, 
                                      channel, height + filter_height-1, height + filter_height-1);
  if( status != CUDNN_STATUS_SUCCESS) {
...
// workspace
  ...
  Util::PrintTensor(HostOutputTensor, batch, channel, height + filter_height-1, height + filter_height-1);
