%%writefile cudnn_batchnormalForwardInference_test.cc
#include "cudnn_batchnormalForwardInference_test.h"
#include "cudnn_utility.h"

#define ExponentialAverageFactor 0.5
#define EPSILON 0.001

BatchNormalizationForwardInference::BatchNormalizationForwardInference(int batch,
    int channel, int height, int width, char *mode) : batch(batch),
    channel(channel), height(height), width(width), mode(mode) {}

void BatchNormalizationForwardInference::FreeMemory() {
  if (HostInputTensor) {
...
int BatchNormalizationForwardInference::BatchNormalizationForwardInferenceApiCall() {
  ...
  cudnnBatchNormalizationForwardTraining(handle_, bn_mode,
  ...
  status = cudnnBatchNormalizationForwardInference(handle_, bn_mode,
                &alpha, &beta, input_desc, DeviceInputTensor, output_desc,
                DeviceOutputTensor, mean_descriptor, device_scale,
                device_offset, device_running_mean,
                device_running_var,EPSILON);
...
int main(int argc, char** argv) {
  ...
  BatchNormalizationForwardInference batchnormalization(batch, channel, height, width, mode);
  batchnormalization.BatchNormalizationForwardInferenceApiCall();
}
