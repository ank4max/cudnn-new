...
#include "cublas_Csyrk3mEx_test.h"

template<class T>
Csyrk3mEx<T>::Csyrk3mEx(int A_row, int A_col, int C_row, int C_col, T alpha, T beta, char mode)
    : A_row(A_row), A_col(A_col), C_row(C_row), C_col(C_col), alpha(alpha), beta(beta), mode(mode) {}

template<class T>
void Csyrk3mEx<T>::FreeMemory() {
  //! Free Host Memory
  if (HostMatrixA)
    delete[] HostMatrixA;
...
template<class T>
int Csyrk3mEx<T>::Csyrk3mExApiCall() {
   ...
   * This function is an extension of cublasCsyrk where the input matrix and output matrix can have a lower precision but the computation 
  ...
  switch (mode) {
    case 'C': {
      std::cout << "\nCalling Csyrk3mEx API\n";
      clk_start = clock();
      
      status = cublasCsyrk3mEx(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
                              A_row, A_col, &alpha,
                              (cuComplex *)DeviceMatrixA, CUDA_C_32F, A_row,
                              &beta,
                              (cuComplex *)DeviceMatrixC, CUDA_C_32F, C_row);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  Csyrk3mEx kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Csyrk3mEx API call ended\n";
      break;
    }
  ...
  std::cout << "\nMatrix C after " << "Csyrk3mEx operation is:\n";
...
int mode_C(int A_row, int A_col, int C_row, int C_col, float alpha_real, float alpha_imaginary,
          float beta_real, float beta_imaginary) { 

  cuComplex alpha = {(float)alpha_real, (float)alpha_imaginary};
  cuComplex beta = {(float)beta_real, (float)beta_imaginary};
  Csyrk3mEx<cuComplex> CCsyrk3mEx(A_row, A_col, C_row, C_col, alpha, beta, 'C');
  return CCsyrk3mEx.Csyrk3mExApiCall();
}


int (*cublas_func_ptr[])(int, int, int, int, float, float, float, float) = {
  mode_C
};
