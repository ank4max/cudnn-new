...
#define SECONDS(milliseconds) milliseconds/1000
...
namespace util {
  ...
  template<class C>
  void PrintSymmetricMatrixXt(C* Matrix, size_t matrix_row, size_t matrix_col) {
    size_t row , col;
    for (row = 0; row < matrix_row; row++) {
      for (col = 0; col < matrix_col; col++) {
        if (row >= col) {
...
template<class C>
  void PrintSymmetricPackedUpperMatrixXt(C* Matrix, size_t matrix_row, size_t matrix_size) {
    size_t row, col, jump;
    size_t row_elments = 1;
    size_t index;
    for (row = 0; row < matrix_row; row++) {
      jump = matrix_row - 1;
      index = row;
  ...
  template<class C>
  void PrintSymmetricPackedUpperComplexMatrixXt(C* Matrix, size_t matrix_row, size_t matrix_size) {
    size_t row, col, jump;
    size_t row_elments = 1;
    size_t index;
    for (row = 0; row < matrix_row; row++) {
...
void PrintRotgOutput(T* scalar_a, T* sine, C * cosine, char mode){
    std::cout << "The result after " <<mode <<"rotg operation : " << std::endl;
    std::cout << "\nThe scalar_a : " << *(scalar_a)  << std::endl;
    std::cout << "\nThe sine element : " << *(sine)  << std::endl;
    std::cout << "\nThe cosine element : " << *(cosine)  << std::endl;
}

template <class T, class C>
void PrintRotgComplexOutput(T * scalar_a, T * sine, C *cosine, char mode){
    std::cout << "The result after " << mode << "rotg operation : " << std::endl;
    std::cout << "\nThe scalar_a real  : " << (*scalar_a).x  << std::endl;
    std::cout << "\nThe scalar_a imaginary  : " << (*scalar_a).y  << std::endl;
    std::cout << "\nThe sine element real  : " << (*sine).x  << std::endl;
    std::cout << "\nThe sine element imaginary  : " << (*sine).y  << std::endl;
