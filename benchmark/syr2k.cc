...
int Syr2k<T>::Syr2kApiCall() {
  ...
  switch (mode) {
    case 'S': {
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  Ssyr2k kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'D': {
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  Dsyr2k kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'C': {
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  Csyr2k kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'Z': {
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  Zsyr2k kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
