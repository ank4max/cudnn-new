...
int Symv<T>::SymvApiCall() {
  ...
  switch (mode) {
    case 'S': {
      ...
      status = cublasSsymv(handle, CUBLAS_FILL_MODE_LOWER, vector_length, (float *)&alpha, (float *)DeviceMatrixA, A_row,
                           (float *)DeviceVectorX, VECTOR_LEADING_DIMENSION, (float *)&beta, (float *)DeviceVectorY, VECTOR_LEADING_DIMENSION);


      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Ssymv kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'D': {
      ...
      status = cublasDsymv(handle, CUBLAS_FILL_MODE_LOWER, vector_length, (double *)&alpha, (double *)DeviceMatrixA, A_row,
                           (double *)DeviceVectorX, VECTOR_LEADING_DIMENSION, (double *)&beta, (double *)DeviceVectorY, VECTOR_LEADING_DIMENSION);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Dsymv kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'C': {
      ...
      status = cublasCsymv(handle, CUBLAS_FILL_MODE_LOWER, vector_length, (cuComplex *)&alpha, (cuComplex *)DeviceMatrixA, A_row,
                           (cuComplex *)DeviceVectorX, VECTOR_LEADING_DIMENSION, (cuComplex *)&beta, (cuComplex *)DeviceVectorY, VECTOR_LEADING_DIMENSION);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Csymv kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'Z': {
      ...
      status = cublasZsymv(handle, CUBLAS_FILL_MODE_LOWER, vector_length, (cuDoubleComplex *)&alpha, (cuDoubleComplex *)DeviceMatrixA, A_row,
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Zsymv kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
  ...
  status = cublasGetVector(vector_length, sizeof (*HostVectorY), DeviceVectorY, 
                           VECTOR_LEADING_DIMENSION, HostVectorY, VECTOR_LEADING_DIMENSION);
