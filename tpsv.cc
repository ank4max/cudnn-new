...
template<class T>
Tpsv<T>::Tpsv(int A_row, int A_col, int vector_length, char mode) : A_row(A_row), A_col(A_col), 
                                                                    vector_length(vector_length), mode(mode) {}

template<class T>
void Tpsv<T>::FreeMemory() {
  //! Free Host Memory
  if (HostMatrixA)
    delete[] HostMatrixA;
...
int Tpsv<T>::TpsvApiCall() {
  ...
  /**
   * API call to solve the packed triangular linear system with a single right-hand-side : \f$ op(A) * X = b \f$ 
  ...
  switch (mode) {
    case 'S': {
      ...
      status = cublasStpsv(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                           A_row, (float *)DeviceMatrixA, (float *)DeviceVectorX, VECTOR_LEADING_DIMENSION);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << " Stpsv kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Stpsv API call ended\n";
      break;
    }
    ...
    case 'D': {
      ...
      status = cublasDtpsv(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                           A_row, (double *)DeviceMatrixA, (double *)DeviceVectorX, VECTOR_LEADING_DIMENSION);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << " Dtpsv kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Dtpsv API call ended\n";
      break;
    }
    ...
    case 'C': {
      ...
      status = cublasCtpsv(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                           A_row, (cuComplex *)DeviceMatrixA, (cuComplex *)DeviceVectorX, VECTOR_LEADING_DIMENSION);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << " Ctpsv kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Ctpsv API call ended\n";
      break;
    }
    ...
    case 'Z': {
      ...
      status = cublasZtpsv(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT,
                           A_row, (cuDoubleComplex *)DeviceMatrixA, (cuDoubleComplex *)DeviceVectorX, VECTOR_LEADING_DIMENSION);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << " Ztpsv kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Ztpsv API call ended\n";
      break;
    }
  }
  ...
  std::cout << "\nVector X after " << mode << "tpsv operation is:\n";
...
void mode_S(int A_row, int A_col, int vector_length) {
  ...
  Tpsv<float> Stpsv(A_row, A_col, vector_length, 'S' );
  Stpsv.TpsvApiCall();
}

void mode_D(int A_row, int A_col, int vector_length) {
            
  Tpsv<double> Dtpsv(A_row, A_col, vector_length, 'D');
  Dtpsv.TpsvApiCall();
}

void mode_C(int A_row, int A_col, int vector_length) {
            
  Tpsv<cuComplex> Ctpsv(A_row, A_col, vector_length, 'C');
  Ctpsv.TpsvApiCall();
}

void mode_Z(int A_row, int A_col, int vector_length) {
            
  Tpsv<cuDoubleComplex> Ztpsv(A_row, A_col, vector_length, 'Z');
  Ztpsv.TpsvApiCall();
}
