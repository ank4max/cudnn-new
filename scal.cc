...
template<class T>
Scal<T>::Scal(int vector_length, T alpha, char mode)
              : vector_length(vector_length), alpha(alpha), mode(mode) {}

template<class T>
void Scal<T>::FreeMemory() {
  //! Free Host Memory
  if (HostVectorX)
    delete[] HostVectorX;
...
int Scal<T>::ScalApiCall() {
  ...
  switch (mode) {
    case 'S': {
      ...
      status = cublasSscal(handle, vector_length, (float *)&alpha, (float *)DeviceVectorX, 1);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Sscal kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'D': {
      ...
      status = cublasDscal(handle, vector_length, (double *)&alpha, (double *)DeviceVectorX, 1);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Dscal kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'C': {
      ...
      status = cublasCscal(handle, vector_length, (cuComplex *)&alpha, (cuComplex *)DeviceVectorX, 1);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Cscal kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'H': {
      ...
      status = cublasCsscal(handle, vector_length, (float *)&alpha, (cuComplex *)DeviceVectorX, 1);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Csscal kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'Z': {
      ...
      status = cublasZscal(handle, vector_length, (cuDoubleComplex *)&alpha, (cuDoubleComplex *)DeviceVectorX, 1);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Zscal kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'T': {
      ...
      status = cublasZdscal(handle, vector_length, (double *)&alpha, (cuDoubleComplex *)DeviceVectorX, 1);

      if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf (stderr, "!!!!  Zdscal kernel execution error\n");
        FreeMemory();
        return EXIT_FAILURE;
      }
...
void mode_S(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  Scal<float> Sscal(vector_length, alpha, 'S' );
  Sscal.ScalApiCall();
}
...
void mode_D(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  Scal<double> Dscal(vector_length, alpha, 'D');
  Dscal.ScalApiCall();
}
...
void mode_C(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  Scal<cuComplex> Cscal(vector_length, alpha, 'C');
  Cscal.ScalApiCall(); 
}
...
void mode_H(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  Scal<cuComplex> Csscal(vector_length, alpha, 'H');
  Csscal.ScalApiCall(); 
}
...
void mode_Z(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  Scal<cuDoubleComplex> Zscal(vector_length, alpha, 'Z');
  Zscal.ScalApiCall(); 
}
...
void mode_T(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  Scal<cuDoubleComplex> Zdscal(vector_length, alpha, 'Z');
  Zdscal.ScalApiCall(); 
}
