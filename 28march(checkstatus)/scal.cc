...
int Scal<T, C>::ScalApiCall() {
  ...
  switch (mode) {
    case 'S': {
      ...
      status = cublasSscal(handle, vector_length, (float *)&alpha,
    ...
    case 'D': {
      ...
      status = cublasDscal(handle, vector_length, (double *)&alpha,
    ...
    case 'C': {
      ...
      status = cublasCscal(handle, vector_length, (cuComplex *)&alpha,
    ...
    case 'H': {
      ...
      status = cublasCsscal(handle, vector_length, (float *)&alpha,
	                    (cuComplex *)DeviceVectorX, VECTOR_LEADING_DIMENSION);
    ...
    case 'Z': {
      ...
      status = cublasZscal(handle, vector_length, (cuDoubleComplex *)&alpha,
	                   (cuDoubleComplex *)DeviceVectorX,
			   VECTOR_LEADING_DIMENSION);
    ...
    case 'T': {
      ...
      status = cublasZdscal(handle, vector_length, (double *)&alpha,
	                    (cuDoubleComplex *)DeviceVectorX,
      			    VECTOR_LEADING_DIMENSION);
...
int mode_Z(int vector_length, double alpha_real, double alpha_imaginary) {
  ...
  return Zscal.ScalApiCall();
}

int mode_T(int vector_length, double alpha_real, double alpha_imaginary) {
  double alpha = alpha_real;

  Scal<cuDoubleComplex, double> Zdscal(vector_length, alpha, 'T');
  return Zdscal.ScalApiCall();
}
