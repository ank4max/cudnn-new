...
Rot<T, C>::Rot(int vector_length, C sine, double cosine, char mode)
    : vector_length(vector_length), sine(sine), cosine(cosine), mode(mode) {}

template<class T, class C>
void Rot<T, C>::FreeMemory() {
  //! Free Host Memory
  if (HostVectorX)
    delete[] HostVectorX;
...
int Rot<T, C>::RotApiCall() {
  ...
  switch (mode) {
    case 'S': {
      ...
      status = cublasSrot(handle, vector_length, (float *)DeviceVectorX,
	                  VECTOR_LEADING_DIMENSION, (float *)DeviceVectorY,
                          VECTOR_LEADING_DIMENSION, (float *)&cosine_f,
			  (float *)&sine);
    ...
    case 'D': {
      ...
      status = cublasDrot(handle, vector_length, (double *)DeviceVectorX,
	                  VECTOR_LEADING_DIMENSION, (double *)DeviceVectorY,
                          VECTOR_LEADING_DIMENSION, (double *)&cosine,
			  (double *)&sine);
    ...
    case 'C': {
      ...
      status = cublasCrot(handle, vector_length, (cuComplex *)DeviceVectorX,
                          VECTOR_LEADING_DIMENSION, (cuComplex *)DeviceVectorY,
                          VECTOR_LEADING_DIMENSION, (float *)&cosine_f,
			  (cuComplex *)&sine);
    ...
    case 'H': {
      ...
      status = cublasCsrot(handle, vector_length, (cuComplex *)DeviceVectorX,
                           VECTOR_LEADING_DIMENSION, (cuComplex *)DeviceVectorY,
                           VECTOR_LEADING_DIMENSION, (float *)&cosine_f,
			   (float *)&sine);
    ...
    case 'Z': {
      ...
      status = cublasZrot(handle, vector_length, (cuDoubleComplex *)DeviceVectorX,
                          VECTOR_LEADING_DIMENSION, (cuDoubleComplex *)DeviceVectorY,
                          VECTOR_LEADING_DIMENSION, (double *)&cosine,
    ...
    case 'T': {
      ...
      status = cublasZdrot(handle, vector_length, (cuDoubleComplex *)DeviceVectorX,
	                   VECTOR_LEADING_DIMENSION, (cuDoubleComplex *)DeviceVectorY,
                           VECTOR_LEADING_DIMENSION, (double *)&cosine,
			   (double *)&sine);
  ...
  switch (mode) {
    ...
    case 'Z': {
      std::cout << "\nVector X after " << mode << "rot operation is:\n";
      util::PrintComplexVector<cuDoubleComplex>((cuDoubleComplex *)HostVectorX,
	                                            vector_length);

      std::cout << "\nVector Y after " << mode << "rot operation is:\n";
      util::PrintComplexVector<cuDoubleComplex>((cuDoubleComplex *)HostVectorY,
	                                            vector_length);
      break;
    }
...
int mode_S(int vector_length, double sine_real, double sine_imaginary, double cosine) {
  float sine = (float)sine_real;

  Rot<float, float> Srot(vector_length, sine, cosine,  'S' );
  return Srot.RotApiCall();
}

int mode_D(int vector_length, double sine_real, double sine_imaginary, double cosine) {
  double sine = sine_real;

  Rot<double, double> Drot(vector_length, sine, cosine, 'D');
  return Drot.RotApiCall();
}

int mode_C(int vector_length, double sine_real, double sine_imaginary, double cosine) {
  cuComplex sine = {(float)sine_real, (float)sine_imaginary};

  Rot<cuComplex, cuComplex> Crot(vector_length, sine, cosine, 'C');
  return Crot.RotApiCall();
}

int mode_H(int vector_length, double sine_real, double sine_imaginary, double cosine) {
  float sine = (float)sine_real;

  Rot<cuComplex, float> Csrot(vector_length, sine, cosine, 'H');
  return Csrot.RotApiCall();
}

int mode_Z(int vector_length, double sine_real, double sine_imaginary, double cosine) {
  cuDoubleComplex sine = {sine_real, sine_imaginary};

  Rot<cuDoubleComplex, cuDoubleComplex> Zrot(vector_length, sine, cosine, 'Z');
  return Zrot.RotApiCall();
}

int mode_T(int vector_length, double sine_real, double sine_imaginary, double cosine) {
  double sine = sine_real;

  Rot<cuDoubleComplex, double> Zdrot(vector_length, sine, cosine, 'T');
  return Zdrot.RotApiCall();
}

int (*cublas_func_ptr[])(int, double, double, double) = {
  mode_S, mode_D, mode_C, mode_H, mode_Z, mode_T
};

int main(int argc, char **argv) {
  int vector_length, status;
  double alpha, alpha_radian, sine_real, sine_imaginary;
  double cosine;
  char mode;
  ...
  sine_imaginary = sin(alpha_radian);
  cosine = cos(alpha_radian);

  status = (*cublas_func_ptr[mode_index[mode]])(vector_length, sine_real, sine_imaginary, cosine);
