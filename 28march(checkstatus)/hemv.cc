%%writefile cublas_hemv_test.cc
#include <unordered_map>
#include "cublas_hemv_test.h"

template<class T>
Hemv<T>::Hemv(int A_row, int A_col, int vector_length, T alpha, T beta,
	char mode) : A_row(A_row), A_col(A_col), vector_length(vector_length),
...
int Hemv<T>::HemvApiCall() {
  ...
  switch (mode) {
    ...
    case 'C': {
      ...
      status = cublasChemv(handle, CUBLAS_FILL_MODE_LOWER, vector_length,
    ...
    case 'Z': {
      ...
      status = cublasZhemv(handle, CUBLAS_FILL_MODE_LOWER, vector_length,
