...
#include "cublas_amax_test.h"

template<class T>
Amax<T>::Amax(int vector_length, char mode)
...
int Amax<T>::AmaxApiCall() {
  ...
  switch (mode) {
    case 'S': {
      ...
      status = cublasIsamax(handle, vector_length, (float *)DeviceVectorX,
                            VECTOR_LEADING_DIMENSION, &result);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  IsAmax kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'D': {
      ...
      status = cublasIdamax(handle, vector_length, (double *)DeviceVectorX,
                            VECTOR_LEADING_DIMENSION, &result);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  IdAmax kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'C': {
      ...
      status = cublasIcamax(handle, vector_length, (cuComplex *)DeviceVectorX, 
                            VECTOR_LEADING_DIMENSION, &result);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  IcAmax kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
    ...
    case 'Z': {
      ...
      status = cublasIzamax(handle, vector_length, (cuDoubleComplex *)DeviceVectorX,
                            VECTOR_LEADING_DIMENSION, &result);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  IzAmax kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
  ...
  std::cout << "\nMaximum value is at index - " << result << std::endl;
...
int mode_S(int vector_length) { 
  Amax<float> SAmax(vector_length, 'S' );
  return SAmax.AmaxApiCall();
}

int mode_D(int vector_length) {
  Amax<double> DAmax(vector_length, 'D');
  return DAmax.AmaxApiCall();
}

int mode_C(int vector_length) {
  Amax<cuComplex> CAmax(vector_length, 'C');
  return CAmax.AmaxApiCall(); 
}

int mode_Z(int vector_length) {
  Amax<cuDoubleComplex> ZAmax(vector_length, 'Z');
  return ZAmax.AmaxApiCall(); 
}
