...
template<class T>
Hbmv<T>::Hbmv(int A_row, int A_col, int vector_length, int sub_diagonals, T alpha, T beta, char mode) : A_row(A_row), A_col(A_col), 
...
int Hbmv<T>::HbmvApiCall() {
  ...
  switch (mode) {
    ...
    case 'C': {
      ...
      status = cublasChbmv(handle, CUBLAS_FILL_MODE_LOWER, A_row, sub_diagonals, (cuComplex *)&alpha, (cuComplex *)DeviceMatrixA, A_row, 
    ...
    case 'Z': {
      ...
      status = cublasZhbmv(handle, CUBLAS_FILL_MODE_LOWER, A_row, sub_diagonals, (cuDoubleComplex *)&alpha, (cuDoubleComplex *)DeviceMatrixA, A_row, 
...
void mode_C(int A_row, int A_col, int vector_length, int sub_diagonals,  double alpha_real, double alpha_imaginary,
...
void mode_Z(int A_row, int A_col, int vector_length, int sub_diagonals, double alpha_real, double alpha_imaginary,
            double beta_real, double beta_imaginary) {
...
int main(int argc, char **argv) {
  ...
  for (int loop_count = 1; loop_count < argc; loop_count += 2) {
    ...
    if (!(cmd_argument.compare("-sub_diagonals")))
  ...
  (*cublas_func_ptr[mode_index[mode]])(A_row, A_col, vector_length, sub_diagonals, alpha_real, alpha_imaginary, 
