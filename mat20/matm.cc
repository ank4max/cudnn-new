...
/*
    case 'D' : {
      //! Creating necessary descriptors
      status = cublasLtMatmulDescInit(&operationDesc, CUBLAS_COMPUTE_64F, CUDA_R_64F);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Operation Descriptor Initialization error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }
      ...
      status = cublasLtMatrixLayoutCreate(&Adesc, CUDA_R_64F, A_row, A_col, A_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for A \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Bdesc, CUDA_R_64F, B_row, B_col, B_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for B \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Cdesc, CUDA_R_64F, C_row, C_col, C_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for C \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Preference Initialization error \n";
        FreeMemory();
        return EXIT_FAILURE;
      }
 ...
 */
    ...
    case 'H': {
      ...
      status = cublasLtMatrixLayoutCreate(&Adesc, CUDA_R_16F, A_row, A_col, A_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for A \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Bdesc, CUDA_R_16F, B_row, B_col, B_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for B \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Cdesc, CUDA_R_16F, C_row, C_col, C_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for C \n";
        FreeMemory();
        return EXIT_FAILURE;
    ...
    case 'C': {
      //! Creating necessary descriptors
      status = cublasLtMatmulDescCreate(&operationDesc, CUBLAS_COMPUTE_32F_FAST_TF32, CUDA_C_32F);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Operation Descriptor creation error\n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Adesc, CUDA_C_32F, A_row, A_col, A_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for A \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Bdesc, CUDA_C_32F, B_row, B_col, B_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for B \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Cdesc, CUDA_C_32F, C_row, C_col, C_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for C \n";
        FreeMemory();
        return EXIT_FAILURE;
    ...
    case 'Z': {
      //! Creating necessary descriptors
      status = cublasLtMatmulDescCreate(&operationDesc, CUBLAS_COMPUTE_64F_PEDANTIC, CUDA_C_64F);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Operation Descriptor creation error\n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Adesc, CUDA_C_64F, A_row, A_col, A_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for A \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Bdesc, CUDA_C_64F, B_row, B_col, B_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for B \n";
        FreeMemory();
        return EXIT_FAILURE;
      ...
      status = cublasLtMatrixLayoutCreate(&Cdesc, CUDA_C_64F, C_row, C_col, C_row);
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "Layout create error for C \n";
        FreeMemory();
        return EXIT_FAILURE;
  ...
  switch (mode) {
    ...
    case 'D': {
      ...
      status = cublasLtMatmul(LtHandle, operationDesc, (double *)&alpha, (double *)DeviceMatrixA, Adesc, (double *)DeviceMatrixB, Bdesc, 
                             (double *)&beta,(double *)DeviceMatrixC, Cdesc, (double *)DeviceMatrixC, Cdesc, &heuristicResult.algo, workspace, workspaceSize, stream);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  DMatmul kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "DMatmul API call ended\n";
      break;
    }
    ...
    case 'H': {
      ...
      status = cublasLtMatmul(LtHandle, operationDesc, (__half *)&alpha, (__half *)DeviceMatrixA, Adesc, (__half *)DeviceMatrixB, Bdesc, 
                              (__half *)&beta, (__half *)DeviceMatrixC, Cdesc, (__half *)DeviceMatrixC, Cdesc, &heuristicResult.algo, workspace, workspaceSize, stream);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  HMatmul kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "HMatmul API call ended\n";
      break;
    }
    ...
    case 'C': {
      ...
      status = cublasLtMatmul(LtHandle, operationDesc, (cuComplex *)&alpha, (cuComplex *)DeviceMatrixA, Adesc, (cuComplex *)DeviceMatrixB, 
                             Bdesc, (cuComplex *)&beta, (cuComplex *)DeviceMatrixC, Cdesc, (cuComplex *)DeviceMatrixC, Cdesc, &heuristicResult.algo, workspace, workspaceSize, stream);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  CMatmul kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "CMatmul API call ended\n";
      break;
    }
    ...
    case 'Z': {
      ...
      status = cublasLtMatmul(LtHandle, operationDesc, (cuDoubleComplex *)&alpha, (cuDoubleComplex *)DeviceMatrixA, Adesc, 
                             (cuDoubleComplex *)DeviceMatrixB, Bdesc, (cuDoubleComplex *)&beta, (cuDoubleComplex *)DeviceMatrixC, Cdesc, 
                             (cuDoubleComplex *)DeviceMatrixC, Cdesc, &heuristicResult.algo, workspace, workspaceSize, stream);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  ZMatmul kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "ZMatmul API call ended\n";
      break;
    }
...
int mode_S(int A_row, int A_col, int B_row, int B_col, int C_row, int C_col, double alpha_real, double alpha_imaginary, 
  ...
  Matmul<float> Smatmul(A_row, A_col, B_row, B_col, C_row, C_col, alpha, beta, 'S' );
  return Smatmul.MatmulApiCall();
}
...
int mode_D(int A_row, int A_col, int B_row, int B_col, int C_row, int C_col, double alpha_real, double alpha_imaginary, 
  ...
  Matmul<double> Dmatmul(A_row, A_col, B_row, B_col, C_row, C_col, alpha, beta, 'D');
  return Dmatmul.MatmulApiCall();
}
...
int mode_C(int A_row, int A_col, int B_row, int B_col, int C_row, int C_col, double alpha_real, double alpha_imaginary,
  ...
  Matmul<cuComplex> Cmatmul(A_row, A_col, B_row, B_col, C_row, C_col, alpha, beta, 'C');
  return Cmatmul.MatmulApiCall();
...
int mode_Z(int A_row, int A_col, int B_row, int B_col, int C_row, int C_col, double alpha_real, double alpha_imaginary,
  ...
  Matmul<cuDoubleComplex> Zmatmul(A_row, A_col, B_row, B_col, C_row, C_col, alpha, beta, 'Z');
  return Zmatmul.MatmulApiCall();
...
int mode_H(int A_row, int A_col, int B_row, int B_col, int C_row, int C_col, double alpha_real, double alpha_imaginary,
  ...
  Matmul<__half> Hmatmul(A_row, A_col, B_row, B_col, C_row, C_col, alpha, beta, 'H');
  return Hmatmul.MatmulApiCall();
}
