...
template<class T>
Spr2<T>::Spr2(int A_row, int A_col, int vector_length, T alpha, char mode)
    : A_row(A_row), A_col(A_col), vector_length(vector_length), 
      alpha(alpha), mode(mode) {}

template<class T>
void Spr2<T>::FreeMemory() {
  //! Free Host Memory
  if (HostMatrixA)
    delete[] HostMatrixA;
...
int Spr2<T>::Spr2ApiCall() {
   ...
   * A is a symmetric packed matrix, X and Y are vectors
   */
  switch (mode) {
    case 'S': {
  ...
  /**
   * API call to perform the packed symmetric rank-2 update : \f$ A = alpha * (X * Y ^ T + Y * X ^ T) + A \f$
   */
  switch (mode) {
    case 'S': {
      std::cout << "\nCalling Sspr2 API\n";
      ...
      status = cublasSspr2(handle, CUBLAS_FILL_MODE_LOWER, A_row, (float *)&alpha, (float *)DeviceVectorX, VECTOR_LEADING_DIMENSION,
                           (float *)DeviceVectorY, VECTOR_LEADING_DIMENSION, (float *)DeviceMatrixA);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << " Sspr2 kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Sspr2 API call ended\n";
      break;
    }
    ...
    case 'D': {
      ...
      status = cublasDspr2(handle, CUBLAS_FILL_MODE_LOWER, A_row, (double *)&alpha, (double *)DeviceVectorX, VECTOR_LEADING_DIMENSION,
                           (double *)DeviceVectorY, VECTOR_LEADING_DIMENSION, (double *)DeviceMatrixA);

      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << " Dspr2 kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Dspr2 API call ended\n";
      break;
    }
  }
  ...
  std::cout << "\nMatrix A after " << mode << "spr2 operation is:\n";
...
void mode_S(int A_row, int A_col, int vector_length, double alpha_real) {
  ...
  Spr2<float> Sspr2(A_row, A_col, vector_length, alpha, 'S');
  Sspr2.Spr2ApiCall();
}
...
void mode_D(int A_row, int A_col, int vector_length, double alpha_real) {   
  ...
  Spr2<double> Dspr2(A_row, A_col, vector_length, alpha, 'D');
  Dspr2.Spr2ApiCall();
}
