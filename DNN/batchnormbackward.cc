...
BatchNormalizationBackward::BatchNormalizationBackward(int batch, int channel, int height, int width, char *mode)
                                                       : batch(batch), channel(channel), height(height), width(width),
                                                       mode(mode) {}

void BatchNormalizationBackward::FreeMemory() {
  if (HostInputTensor) {
  ...
  if( cudaStatus != cudaSuccess) {
    std::cout << " Device result memory deallocation error\n" << std::endl;
  }
...
int BatchNormalizationBackward::BatchNormalizationBackwardApiCall() {
  ...
  cudaStatus = cudaMalloc(&result, size_bytes);
  if(cudaStatus != cudaSuccess) {
    std::cout << " Memory allocation on device for result failed\n" << std::endl;
    FreeMemory();
    return EXIT_FAILURE;
  }
  ...
  clk_start=clock();
  status = cudnnBatchNormalizationBackward(handle_, bn_mode, &alpha, &beta, &alpha, &beta, input_desc, DeviceInputTensor, output_desc,
                                           DeviceOutputTensor, input_desc, result,  mean_descriptor, device_scale, device_running_mean,
                                           device_running_var, EPSILON, device_saved_mean, device_saved_inv_var);
  ...
  cudaStatus = cudaMemcpy(HostOutputTensor, result, size_bytes,
...
int main(int argc, char** argv) {
  ...
  BatchNormalizationBackward batchnormalization(batch, channel, height, width, mode);
  batchnormalization.BatchNormalizationBackwardApiCall();
}
