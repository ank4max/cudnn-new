...
#include "cublas_CsyrkEx_test.h"

template<class T>
CsyrkEx<T>::CsyrkEx(int A_row, int A_col, int C_row, int C_col, T alpha, T beta, char mode)
    : A_row(A_row), A_col(A_col), C_row(C_row), C_col(C_col), alpha(alpha), beta(beta), mode(mode) {}

template<class T>
void CsyrkEx<T>::FreeMemory() {
  //! Free Host Memory
  if (HostMatrixA)
    delete[] HostMatrixA;
...
int CsyrkEx<T>::CsyrkExApiCall() {
  ...
  switch (mode) {
    case 'C': {
      ...
      status = cublasCsyrkEx(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
                              A_row, A_col, &alpha,
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  CsyrkEx kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "CsyrkEx API call ended\n";
      break;
    }
  ...
  std::cout << "\nMatrix C after " << "CsyrkEx operation is:\n";
...
int mode_C(int A_row, int A_col, int C_row, int C_col, float alpha_real, float alpha_imaginary,
  ...
  CsyrkEx<cuComplex> CCsyrkEx(A_row, A_col, C_row, C_col, alpha, beta, 'C');
  return CCsyrkEx.CsyrkExApiCall();
}
