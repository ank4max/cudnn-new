...
class TrsmBatched {
  ...
  public:
    ...
    int TrsmBatchedApiCall() {
      ...
      switch (mode) {
        ...
        case 'C': {
          ...
          status = cublasCtrsmBatched(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, 
                              CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, A_row, B_col, 
                              (const cuComplex *)&alpha, (cuComplex* const*)DeviceMatrixA, A_row, 
        ...
        case 'Z': {
          ...
          status = cublasZtrsmBatched(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, 
                              CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, A_row, B_col, 
                              (const cuDoubleComplex *)&alpha, (cuDoubleComplex* const*)DeviceMatrixA, A_row, 
      ...
      for (batch = 0; batch < batch_count; batch++) {
        status = cublasGetMatrix(B_row, B_col, sizeof(T), HostPtrToDeviceMatB[batch], B_row, HostMatrixB[batch], B_row);
        if (status != CUBLAS_STATUS_SUCCESS) {
