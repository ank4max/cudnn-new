...
#include "cublas_Cherk3mEx_test.h"

template<class T>
Cherk3mEx<T>::Cherk3mEx(int A_row, int A_col, int C_row, int C_col, float alpha, float beta, char mode)
    : A_row(A_row), A_col(A_col), C_row(C_row), C_col(C_col), alpha(alpha), beta(beta), mode(mode) {}

template<class T>
void Cherk3mEx<T>::FreeMemory() {
  //! Free Host Memory
  if (HostMatrixA)
    delete[] HostMatrixA;
...
template<class T>
int Cherk3mEx<T>::Cherk3mExApiCall() {
  ...
  switch (mode) {
    case 'C': { 
      std::cout << "\nCalling Cherk3mEx API\n";
      clk_start = clock();

      status = cublasCherk3mEx(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
      ...
      if (status != CUBLAS_STATUS_SUCCESS) {
        std::cout << "!!!!  Cherk3mEx kernel execution error\n";
        FreeMemory();
        return EXIT_FAILURE;
      }

      clk_end = clock();
      std::cout << "Cherk3mEx API call ended\n";
      break;
    }
  ...
  std::cout << "\nMatrix C after " << "Cherk3mEx operation is:\n";
...
int mode_C(int A_row, int A_col, int C_row, int C_col, float alpha_real, float beta_real) {
  ...
  Cherk3mEx<cuComplex> CCherk3mEx(A_row, A_col, C_row, C_col, alpha_real, beta_real, 'C');
  return CCherk3mEx.Cherk3mExApiCall();
